...
- **Still prefer the app's own name and a direct `url(data:…)` for anything new**, even though the variables happen to work: the reason is not that they are missing, it is that **the server resolves its own identifiers per theme and a note does not follow the theme**. Two families, and the difference decides everything below — measured by rendering each glyph to a canvas and averaging its luma, over the six combinations of (theme picked) x (what the browser prefers):
  - `--original-icon-*` has **one** definition in the server's `dist/icons.css` and is never swapped. Always the asset its name says. `.slim-tag` and `.icon-delete-note` lean on it and are safe on a light note.
  - The bare `icon-<name>` classes go through `--icon-<name>-dark`, which `dist/icons.css` defines **four** times: on `body` (the light asset), on `body` inside `@media (prefers-color-scheme: dark)`, and again under `[data-themes*=light]` and `[data-themes*=dark]`. The attribute selectors are (0,1,0) against (0,0,1), so a theme the user picked by hand wins over the media query, and the media query only gets to decide when the theme is left at *system* — exactly how `--background-invert-if-dark` behaves. Measured: `icon-tag`, `icon-toggle-background`, `icon-picture` and `icon-delete` come out white under a dark theme whatever the browser prefers, dark under a light one, and follow the browser under *system*.
  So a server icon is **right wherever the background follows the theme** — the sidebar, `#app-content`, `#trash-bar` — and wrong only where it does not. `icon-qn-share`, `icon-qn-shared`, `icon-qn-reminder`, `icon-qn-tag`, `icon-qn-colors`, `icon-qn-picture`, `icon-qn-trash`, `.slim-share`, `.slim-share-group`, `.slim-reminder` and `.attach-remove` are the app's own, and are dark in every theme.
- **A new icon needs a name the server does not use and a direct `url(data:…)`** — never a bare `icon-<something-nextcloud-has>` and never `var(--original-icon-…)`. Those identifiers belong to the server: where it ships its icon stylesheet it defines `body .icon-calendar` itself, which at specificity (0,1,1) beats a plain `.icon-calendar` in the app's CSS and resolves to whichever asset the current theme wants — a white one under a dark theme, on a sidebar or a note that is not dark. It fails *differently* depending on whether the instance serves that stylesheet, which makes it miserable to debug. Every icon of the app that renders correctly avoids both traps: `icon-pin`, `icon-pinned`, `icon-archived`, `icon-unarchive`, `icon-restore`, `icon-qn-reminder`, `icon-qn-share`, `icon-qn-shared`, `icon-qn-tag`, `icon-qn-colors`, `icon-qn-picture`, `icon-qn-trash`, `.slim-reminder` and `.slim-share`. The navigation is the one place that *does* follow the theme, and it is handled on purpose: the icon is painted on the `a::before` (`#app-navigation` gets `background-image: none; filter: none` on the entry) and the `--background-invert-if-dark` filter lives on that pseudo-element only — a filter on the `a` itself inverts the label text along with the glyph and makes the sidebar unreadable in dark mode, which is what a single `#app-navigation .icon-qn-share { filter }` rule did until 1.0. The per-icon `::before` backgrounds live in `css/icons.css`, the base rule in `css/not-vue.css`. The same filter on a *textless* element is fine: the empty states of the grid sit on the theme background, so `.emptycontent .icon-archived` and `.emptycontent .icon-qn-trash` filter the element itself.
- **A new icon should also be a plain dark svg, with no `filter` and no media query**, like all of the above. Two things that seem like the right call and are not, for a single icon:
  - `@media (prefers-color-scheme: dark)`, which follows the **browser** alone, and so disagrees with the theme whenever the user picked one by hand. Nextcloud itself never relies on it alone: `theming` injects every theme twice (`ThemeInjectionService::injectHeaders()`) — as `:root` under `media="(prefers-color-scheme: dark)"`, and as `body[data-theme-<id>]` with no media query — and `dist/icons.css` mirrors that with its `[data-themes*=…]` blocks. Verified in a browser: `body[data-themes]` reads `dark`, `light` or the empty string, and only the empty string leaves the media query in charge. A lone media query in this app is therefore always wrong; `icon-quicknotes` had one until 1.0, which is why a dark theme on a light OS drew its dark glyph on the dark sidebar.
  - `filter: var(--background-invert-if-dark)`, which does follow the theme (`invert(100%)` under a dark theme, the deliberately invalid `no` otherwise — measured in all six combinations) but only makes sense where the background follows the theme too, and never on an element that also carries text. A **note keeps its light colour in every theme**, so anything sitting on one must stay dark unconditionally. That is also why the editor toolbar could not keep the server's `icon-toggle-background`, `icon-tag` and `icon-picture`: under a dark theme they turn white, on a card hard-coded to `#F7EB96`, and nothing about the theme can rescue them. They are `icon-qn-colors`, `icon-qn-tag` and `icon-qn-picture` now. `#empty-trash` keeps the server's `icon-delete` on purpose — `#trash-bar` sits on `#app-content`, which does follow the theme, so there the server's answer is the right one. The three sidebar entries that used to be server icons (`icon-tag`, `icon-toggle-background`, `icon-delete`) were **not** broken for this reason; they are app icons now so that every glyph in the navigation comes from one place and through one mechanism, which is what the `::before` below is.
